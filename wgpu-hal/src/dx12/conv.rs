...
pub(super) fn map_texture_format(format: wgt::TextureFormat) -> Dxgi::DXGI_FORMAT {
...
pub fn map_texture_format_nosrgb(format: wgt::TextureFormat) -> Dxgi::DXGI_FORMAT {
    match format {
        wgt::TextureFormat::Bgra8UnormSrgb => Dxgi::DXGI_FORMAT_B8G8R8A8_UNORM,
        wgt::TextureFormat::Rgba8UnormSrgb => Dxgi::DXGI_FORMAT_R8G8B8A8_UNORM,
...
//TODO: stencil views?
pub fn map_texture_format_nodepth(format: wgt::TextureFormat) -> Dxgi::DXGI_FORMAT {
    match format {
        wgt::TextureFormat::Depth32Float => Dxgi::DXGI_FORMAT_R32_FLOAT,
...
pub fn map_texture_format_depth_typeless(format: wgt::TextureFormat) -> Dxgi::DXGI_FORMAT {
    match format {
        wgt::TextureFormat::Depth32Float => Dxgi::DXGI_FORMAT_R32_TYPELESS,
        wgt::TextureFormat::Depth24Plus | wgt::TextureFormat::Depth24PlusStencil8 => {
...
pub fn map_index_format(format: wgt::IndexFormat) -> Dxgi::DXGI_FORMAT {
    match format {
        wgt::IndexFormat::Uint16 => Dxgi::DXGI_FORMAT_R16_UINT,
        wgt::IndexFormat::Uint32 => Dxgi::DXGI_FORMAT_R32_UINT,
    }
}

pub fn map_vertex_format(format: wgt::VertexFormat) -> Dxgi::DXGI_FORMAT {
...
pub fn map_acomposite_alpha_mode(mode: crate::CompositeAlphaMode) -> Dxgi::DXGI_ALPHA_MODE {
    use crate::CompositeAlphaMode as Cam;
    match mode {
        Cam::Opaque => Dxgi::DXGI_ALPHA_MODE_IGNORE,
        Cam::PreMultiplied => Dxgi::DXGI_ALPHA_MODE_PREMULTIPLIED,
        Cam::PostMultiplied => Dxgi::DXGI_ALPHA_MODE_STRAIGHT,
    }
}
...
pub fn map_buffer_usage_to_resource_flags(
    ...
    if usage.contains(crate::BufferUses::STORAGE_WRITE) {
        flags |= Direct3D12::D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS.0;
    }
    Direct3D12::D3D12_RESOURCE_FLAGS(flags)
}
...
pub fn map_texture_usage_to_resource_flags(
    ...
    if usage.contains(crate::TextureUses::COLOR_TARGET) {
        flags |= Direct3D12::D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET.0;
    }
    if usage.intersects(
        crate::TextureUses::DEPTH_STENCIL_READ | crate::TextureUses::DEPTH_STENCIL_WRITE,
    ) {
        flags |= Direct3D12::D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL.0;
        if !usage.contains(crate::TextureUses::RESOURCE) {
            flags |= Direct3D12::D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE.0;
        }
    }
...
fn map_blend_factor(factor: wgt::BlendFactor, is_alpha: bool) -> Direct3D12::D3D12_BLEND {
    ...
    match factor {
        ...
        //Bf::Src1Color if is_alpha => Direct3D12::D3D12_BLEND_SRC1_ALPHA,
        //Bf::Src1Color => Direct3D12::D3D12_BLEND_SRC1_COLOR,
        //Bf::OneMinusSrc1Color if is_alpha => Direct3D12::D3D12_BLEND_INV_SRC1_ALPHA,
        //Bf::OneMinusSrc1Color => Direct3D12::D3D12_BLEND_INV_SRC1_COLOR,
...
pub fn map_render_targets(
    ...
    let dummy_target = Direct3D12::D3D12_RENDER_TARGET_BLEND_DESC {
        BlendEnable: Foundation::BOOL(0),
        LogicOpEnable: Foundation::BOOL(0),
    ...
    for (raw, ct) in raw_targets.iter_mut().zip(color_targets.iter()) {
        ...
        if let Some(ref blend) = ct.blend {
            ...
            raw.BlendEnable = Foundation::BOOL(1);
...
pub fn map_depth_stencil(ds: &wgt::DepthStencilState) -> Direct3D12::D3D12_DEPTH_STENCIL_DESC {
    ...
    Direct3D12::D3D12_DEPTH_STENCIL_DESC {
        DepthEnable: if ds.is_depth_enabled() {
            Foundation::BOOL(1)
        } else {
            Foundation::BOOL(0)
        },
        ...
        StencilEnable: if ds.stencil.is_enabled() {
            Foundation::BOOL(1)
        } else {
            Foundation::BOOL(0)
        },
